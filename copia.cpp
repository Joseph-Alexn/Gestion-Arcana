...
public:
    ...
    Vertice(char runa = '\0') : runa(runa), aristas(Lista<Arista>()) {}
    ~Vertice()
    {
    }
};
...
Hechizo Entrada(const char *nombreArchivo, Lista<Hechizo> listaHechizos)
{
    ...
    for (int i = 0; i < numeroGrafos; i++)
    {
        ...
        listaHechizos.insertarFinal(Hechizo(cantidadVertices));
        Nodo<Hechizo> *ptr_hechizo = listaHechizos.ptr_ultimo;
        ptr_hechizo->valor.nombreMago = nombreMago;
        ptr_hechizo->valor.nombreHechizo = "Hechizo " + to_string(i + 1);
        ...
        {
            ptr_hechizo->valor.obtenerVertice(j) = Vertice(runas[j]);
        }
        ...
        for (int j = 0; j < numeroAristas; j++)
        {
            ...
            {
                ptr_hechizo->valor.obtenerVertice(vertice1).aristas.insertarFinal(Arista(vertice2, peso));
                ptr_hechizo->valor.obtenerVertice(vertice2).aristas.insertarFinal(Arista(vertice1, peso));
            }
            else
            {
        ...
        procesarHechizo(ptr_hechizo->valor);
    }
...
int main()
{
    Lista<Hechizo> listaHechizos;
    Entrada("spellList.in", listaHechizos);
    cout << endl;
    Nodo<Hechizo> *iterador = listaHechizos.ptr_primero;
    while (iterador != nullptr)
    {
        cout << "Nombre del mago: " << iterador->valor.nombreMago << endl;
        cout << "Nombre del hechizo: " << iterador->valor.nombreHechizo << endl;
        cout << "Cantidad de vertices: " << iterador->valor.cantidadVertices << endl;
        iterador = iterador->ptr_siguiente;
    }
    return 0;
}
